...
| ~~Narrative delivery~~ | `narrative.md §5` — opening panels + between-level beats (`decisions_needed.md` D13, rescheduled for 100 levels by D16) | **Runtime built, SPRINT_032** — `src/narrative/` (beat registry, 100-level schedule, `CutscenePlayer`), `NarrativeBeatEvent`/`NarrativeSystem`, `NarrativeState` persistence, `CUTSCENE_ADVANCE` co-op sync. Artwork still outstanding: `public/cutscenes/` is empty, so panels render as placeholders |
...
4. ~~**The Generator ("Deep Coma" / "Daily Synapse").**~~ **v1 done (SPRINT_029), v2 done (SPRINT_030, 2026-07-24), v3 done (SPRINT_031, 2026-07-25)** — `LevelGenerator.ts` + `scripts/generateLevel.ts`, now chaining up to **three** simultaneous required core abilities drawn from all six mechanics (JUMP/PUSH/PHASE_SHIFT now fully wired, not just UNLOCK_RED/UNLOCK_BLUE/FIRE_IMMUNITY), with a scaling `gridRadius` (`generative_levels.md §3.0` discloses the cuts vs. the full spec), genuinely solver/replay/real-browser-verified end to end. SPRINT_029 also closed a real, self-flagged gap: neither prior solvability gate touched the DOM at all — a new Playwright-based real-browser check (`e2e/`) now does, wired into the Generator's own acceptance gate; SPRINT_031 found and fixed a genuine latent bug that gate exists to catch — a global mouse-click listener with no target check could misread HTML-overlay clicks as hex-board moves once high-`gridRadius` boards pushed hex content into the same screen region as UI chrome (`MouseInput.ts`, `LegendPanel.ts`). Batch 1 (v1, levels 30-39) plateaued around D≈6.2; Batch 2 (v2, levels 40-49) reaches D≈7.8 by breaking past the single-ability ceiling; Batch 3 (v3, levels 50-59) adds the new terrain mechanics and the campaign's first genuine 3-ability levels. Still open: endless mode/Daily Synapse's live delivery (separate decision, no backend server exists), arbitrary N-ability topology beyond 3, `gridRadius`>5, 🔢 Chris's calibration of the margin/difficulty curve.
