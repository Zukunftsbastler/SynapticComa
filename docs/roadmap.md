...
4. ~~**The Generator ("Deep Coma" / "Daily Synapse").**~~ **v1 done (SPRINT_029, 2026-07-23)** — `LevelGenerator.ts` + `scripts/generateLevel.ts`, scoped to one required core ability per level (`generative_levels.md §3.0` discloses the cuts vs. the full spec) but genuinely solver/replay/real-browser-verified end to end. Same sprint also closed a real, self-flagged gap: neither prior solvability gate touched the DOM at all — a new Playwright-based real-browser check (`e2e/`) now does, and it's wired into the Generator's own acceptance gate. Still open: endless mode/Daily Synapse's live delivery (separate decision, no backend server exists), arbitrary multi-ability topology, JUMP/PUSH/PHASE_SHIFT hex-layout support, 🔢 Chris's calibration of the margin/difficulty curve.
