...
### Post-MVP (Deferred — Levels 60+)
* **Levels 27–29 (Neuro-Resonance, SPRINT_028 — done):** All four Neuro-Resonance effects are now demonstrated in the campaign. Level 27 "Steady Hand" (Anchor), level 28 "Muscle Memory" (Dampening), and level 29 "Second Sight" (Clarity, kept genuinely non-load-bearing — the solver's proven-optimal path never touches it) join level 26's "First Spark" (Discharge). No engine changes were needed — `ResonanceSystem.ts`/`LevelSolver.ts` already modeled all four pairs from SPRINT_026.
* **Levels 30–39 (Generator Batch 1, SPRINT_029 follow-up — done):** The first ten **procedurally generated** levels, not hand-authored — `scripts/generateLevel.ts`, one required core ability per level (RED/BLUE/FIRE) plus an increasing mix of Resonance/Focus Vault/Echo Tile, solver + headless-replay + real-browser verified via the same triple gate every generated level must clear. Difficulty rose across the batch but **plateaued around D≈6.2**, not strictly monotonically with the requested target — v1's single-ability template had a real, disclosed AP-margin floor (`generative_levels.md §3.0`) below which the level becomes outright unsolvable, not just harder; the generator detects and respects this rather than emitting something broken.
* **Levels 40–49 (Generator Batch 2 / v2, SPRINT_030 — done):** The Generator extended to chain **up to two** simultaneous required core abilities and to scale `gridRadius` (3→5) with requested difficulty, reusing the exact two-gate shape `level_16.json` "Airlock" already ships (mirrored across both dimensions, anchored relative to spawn/exit rather than fixed coordinates). This is what actually breaks Batch 1's D≈6.2 ceiling — achieved difficulty across this batch reaches D≈7.8, and levels visibly grow (radius up to 5, two matrix nodes, longer paths) as requested difficulty rises, the concrete first real step toward Till's stated goal (2026-07-23) of generating all 100 levels with rising difficulty, growing size, and combined mechanics. Still capped at 2 abilities and radius≤5 — arbitrary N-ability graphs, JUMP/PUSH/PHASE_SHIFT hex-layout, and a 3rd physical gate ring remain open (`generative_levels.md §3.0`).
* **Levels 50–59 (Generator Batch 3 / v3, SPRINT_031 — done):** Till's follow-up authorization ("lass uns in 3-Mechaniken-Ketten und passendes Terrain investieren. Es darf mehr Aufwand kosten!") closed v2's remaining disclosed gap — `JUMP`/`PUSH`/`PHASE_SHIFT` are now fully wired into the Generator's hex-layout step (a `phase_barrier` reuses the hazard-gate template; `PUSH` gets new terrain, a `pushable_block` with a narrow 2-wall flank; `JUMP` reuses a 2-hex chasm with the same narrow flank), and ability chaining extends to **up to three** simultaneous required abilities via a third, center-corridor gate slot. Wall placement switched from v1/v2's fixed-width flank to full-row sealing, found necessary once `gridRadius>3` levels revealed hex rows widen away from the poles enough to walk around a fixed flank. `JUMP` stays solo-only — once powered it bypasses the intermediate hex of any 2-hex jump, including a different gate's hazard 2 rows away, so it can't safely share a corridor with other row-gated mechanics under this template. Levels 55/56/59 are the campaign's first genuine 3-ability levels (`needs=[BLUE,FIRE,RED]` etc.), D up to 7.79 within the new terrain mix. Full detail: `generative_levels.md §3.0`, `SPRINTS/SPRINT_031-...md`.
* **Levels 60–64 (Spatial Complexity):** Larger hex grids beyond what the Generator's v3 template reaches, or arbitrary N-ability dependency graphs beyond 3. Navigation to conduits requires long-horizon AP planning across multiple Shared Unlocks. Remaining `mechanic_roadmap.md` proposals (Synaptic Fatigue, Pulse Gates, Scar Tissue, Bruised Fragments, Short Circuit, Static Field, Convergence Nodes) are candidates for this range, pending team selection. **Two concrete concepts from Till (2026-07-23), both buildable with existing mechanics, no new engine work needed:** (1) a maze-like level reachable only via 3–4 well-placed Shared Unlock AP refills scattered through it, combined with other mechanisms already in play; (2) levels with multiple viable-looking routes to a floor `Collectible` (any bonus — AP, an ability plate, a key) where only one route's move-count actually fits the AP budget — exactly what the solver's `optimal`/`slack` columns already score per level, just not yet deliberately designed *around* as the puzzle's centerpiece.
* **Levels 65–70+ (The Deep Subconscious):** Asymmetric foresight clues become subtle and indirect. End-game puzzles designed for experienced cooperative pairs.
