...
> **Status update (SPRINT_019/020/025):** Till selected Part 1's recommendations plus three Part 2 proposals for implementation across several sprints. **Recommendations 2–3 are done** (level 11 redesigned, level 18 "Ghost Step" added for Phase Shift, five new tutorial popups). **Recommendation 1 (Resonance) is formally deferred**, not built (see `mechanics.md §4.5`'s status note). **Mechanics #2 (Impulse Blocks), #8 (Focus Vault), and #3 (Echo Tiles) are implemented** — levels 22, 23, and 25 respectively; see their entries below for what shipped vs. what was originally proposed. The other 7 mechanics remain concept-only, unranked, for future team selection. Part 1's audit below describes the campaign **as it stood before SPRINT_020**; see `level_design.md`'s campaign table for the current 25-level state.
