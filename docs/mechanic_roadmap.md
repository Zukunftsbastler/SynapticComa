...
> **Status update (SPRINT_019/020/025/026):** Till selected Part 1's recommendations plus three Part 2 proposals for implementation across several sprints. **Recommendations 2–3 are done** (level 11 redesigned, level 18 "Ghost Step" added for Phase Shift, five new tutorial popups). **Recommendation 1 (Resonance) is now built** (SPRINT_026, D15 in `decisions_needed.md` — Till decided alone; see `mechanics.md §4.5`'s status note for the scoped-down shape), demonstrated in level 26 "First Spark"; F2's Threshold was formally cut in the same sprint, not revived. **Mechanics #2 (Impulse Blocks), #8 (Focus Vault), and #3 (Echo Tiles) are implemented** — levels 22, 23, and 25 respectively; see their entries below for what shipped vs. what was originally proposed. The other 7 mechanics remain concept-only, unranked, for future team selection. Part 1's audit below describes the campaign **as it stood before SPRINT_020**; see `level_design.md`'s campaign table for the current 26-level state.
...
### 3. Echo Tiles — ✅ IMPLEMENTED (SPRINT_025, level 25 "Thin Place")

*As built: simpler than even the "likely cheap" estimate in `roadmap.md` §4 — rather than a new mini hex-grid renderer, the reveal reuses the existing `GameState.revealBothDims` flag (already fully built for local single-machine testing, and verified to have zero effect outside rendering before reuse). Standing on the tile forces it true for ~3 seconds after last contact, then restores whatever it was before. No network message, no solver awareness — an entity nothing in the required solution touches, same safety argument as Focus Vault.*
...
|---|----------|---------|--------------------------|-------------------|
| 1 | Synaptic Fatigue | AP/ability economy | Jump, Phase/Fire, Resonance (built SPRINT_026) | Overused thought, mental strain |
| 2 | Impulse Blocks | Hex-grid physical space | Jump, Fire/Laser; activates dormant PUSH | Repressed clot / logic block |
| 3 | Echo Tiles | Cross-dimension information | Pulse Gates (Threshold, its original pairing idea, was cut SPRINT_026) | The one other thin place between minds |
...
**Cross-cutting recommendation (resolved SPRINT_019, extended SPRINT_025):** items **2 (Impulse Blocks)**, **8 (Focus Vault)**, and **3 (Echo Tiles)** were the safest picks — all additive-only (no existing level's solvability proof changed), and Echo Tiles turned out cheaper still than estimated (reused an existing rendering flag rather than needing new UI). All three are now implemented (see their entries above). The remaining seven are still a genuine "pick for the team," not a ranked queue — a design-taste call for Till, Andreas, and Chris together.
