...
> **Status (SPRINT_031, 2026-07-25):** Till, after seeing v2's result, authorized the larger remaining investment outright: "lass uns in 3-Mechaniken-Ketten und passendes Terrain investieren. Es darf mehr Aufwand kosten!" The Generator now wires up **all six** core mechanics — `JUMP`, `PUSH`, and `PHASE_SHIFT` join `UNLOCK_RED`/`UNLOCK_BLUE`/`FIRE_IMMUNITY` — and chains **up to three** simultaneous required abilities (up from two). `JUMP`/`PHASE_SHIFT` gates reuse v2's proven hazard-gate wall-funnel (a `phase_barrier` entity is mechanically identical to a hazard door in `LevelSolver.ts`); `PUSH` gates are new terrain (a `pushable_block` at the gate center, 2-wall flank). All wall placement moved to **full-row sealing** (`sealRow` — every non-corridor hex at a gate's row is walled, computed from the hex-distance boundary formula) after empirically finding v2's fixed-width flank leaves rows *wider* than the flank open at `gridRadius>3`, since hex rows widen away from the poles. **`JUMP` is solo-only** — a second, independent finding: once powered, a jump bypasses the intermediate hex entirely, so it can leap over any OTHER row-gated mechanic 2 rows away, not just its own chasm; excluded from multi-ability combos, still fully supported alone. 3-ability mode adds a third physical gate at the corridor's center (independent per-row matrix stacking, no shared-column pass-through). Generated **Batch 3** (levels 50–59); found and fixed a real, previously-latent input-handling bug along the way (`MouseInput.ts`'s global click listener had no target check, so HTML-overlay clicks — inventory slots, matrix insert arrows, the always-present Legend panel — could bubble up and get misread as hex-board moves whenever the overlay's screen position aliased a valid adjacent hex; only surfaced now because higher `gridRadius` boards push hex content further into the same screen region as UI chrome). 59/59 levels green, 60/60 real-browser checks green. Full detail: `SPRINTS/SPRINT_031-...md`.
>
...
The Solver and Difficulty Scorer live in `src/generation/` and are **plain TypeScript, not ECS systems**; they operate on `LevelDef` data (the JSON schema) and a lightweight abstract game state — never on the live bitECS world. The Generator is built (see status notes above) in a still-scoped-down form — §3.0 discloses exactly which parts of this diagram are real vs. simplified.
...
### 3.0 As Shipped (SPRINT_029, extended SPRINT_030, extended SPRINT_031) vs. This Spec

`LevelGenerator.ts` implements steps 1-2-6 essentially as specced, and a **scoped-down** version of 3-4-5:

- **Step 1 (requirement graph):** chains **up to three** required core abilities per generated level (`computeAbilityCount`, SPRINT_031 — v1 shipped with exactly one, v2 with two; still not an arbitrary N-ability dependency graph), drawn from all **six** core mechanics: `UNLOCK_RED`/`UNLOCK_BLUE`/`FIRE_IMMUNITY`/`PHASE_SHIFT`/`JUMP`/`PUSH`. `JUMP` is excluded from any multi-ability combo — see below — and the generator falls back to 1-ability mode if the non-`JUMP` pool can't fill the requested count.
- **Step 3 (hex paths):** the generator reuses proven gate templates rather than walking arbitrary terrain, dispatched per mechanic on a small tagged union (`GateSpec`): **hazard-gate** (RED/BLUE/FIRE/PHASE — a `hazard` or `phase_barrier` entity at the gate's center hex; `phase_barrier` blocks passage identically to a hazard door in `LevelSolver.ts`, including as a push-destination) or **terrain-gate** (JUMP/PUSH — a `hazard{hazardType:CHASM,alwaysLethal}` two hexes from the entry point along the travel axis for JUMP, a `pushable_block` at the gate center for PUSH; push direction is always toward exit, since `LevelSolver.ts` never checks board edges for a push destination). 1/2/3-ability levels get 1/2/3 evenly-spaced gate slots (exit-relative; spawn-relative; a new center slot at `r=0`), each mirrored across both dimensions with the same requirement — no per-dimension asymmetry, no collectibles, no shared-column pass-through (`level_17.json` "Signal Chain"'s hand-authored shape is a different, simpler-to-reason-about mechanism: each gate's ability node just stacks at its own independent `col3row{i}`). **Wall placement changed from a fixed-width flank to full-row sealing** (`sealRow`): every hex at the gate's row that isn't the `q=0` corridor is walled, computed directly from the hex-distance-from-origin boundary formula — found empirically necessary because hex rows *widen* away from the poles (a fixed 2- or 5-wall flank leaves the rest of a wider row open to walk around once `gridRadius>3`), and it's provably airtight at any radius since crossing that row requires passing through `q=0`. **`JUMP` is solo-only, a second empirical finding**: once powered, `LevelSolver.ts`'s jump branch never checks the intermediate hex of a 2-hex jump, so a powered JUMP can bypass *any* other row-gated mechanic exactly 2 rows away, not just its own chasm — this makes it structurally incompatible with sharing a corridor with other row-gated mechanics under this mirrored-per-dimension template. A defensive collision check (`buildDraft`) still rejects any attempt where two entities land on the same hex, same as v2.
...
3. **Generator v1→v3** (SPRINT_029 → SPRINT_030 → SPRINT_031, 2026-07-23 → 2026-07-25): the offline generate-and-verify CLI, scoped down per §3.0 — v3 wires up all six core mechanics and chains up to 3 simultaneous required abilities. Still open: endless mode / Daily Synapse's live delivery mechanism (a separate, later decision — this client is P2P-only with no backend server), arbitrary N-ability/arbitrary-topology generality beyond 3, `gridRadius>5`, and 🔢 Chris's calibration of `computeMargin`/the difficulty curve against real playtests.
